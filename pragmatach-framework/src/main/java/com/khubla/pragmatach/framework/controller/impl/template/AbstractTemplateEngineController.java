...
public class AbstractTemplateEngineController extends AbstractController {
	...
	private String getTemplateAsString(InputStream templateInputStream)
	...
	protected Map<String, Object> getTemplateContext() {
		 ...
		 */
		final Hashtable<Class<?>, PragmatachController> sessionControllerInstances = SessionScopedControllers
				.getMap(getRequest().getSession());
		if (null != sessionControllerInstances) {
			final Enumeration<Class<?>> enumer = sessionControllerInstances
					.keys();
			while (enumer.hasMoreElements()) {
				final Class<?> key = enumer.nextElement();
				final Controller controller = key
						.getAnnotation(Controller.class);
				if (controller.scope() == Controller.Scope.session) {
					final PragmatachController pragmatachController = sessionControllerInstances
							.get(key);
		 ...
		 */
		final Map<String, PluginDescriptor> plugins = PluginDescriptors
				.getPlugins();
		if (null != plugins) {
			for (final PluginDescriptor pluginDescriptor : plugins.values()) {
				final Map<String, Object> vars = pluginDescriptor.getPlugin()
	...
	public String url_for(String page) throws PragmatachException {
		try {
			...
			final URI uri = new URI(httpServletRequest.getRequestURL()
					.toString()).resolve(page);
			return uri.toString();
		} catch (final Exception e) {
			throw new PragmatachException("Exception in getTemplateName", e);
		}
