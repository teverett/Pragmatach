...
public class RouteFinder {
		...
		try {
			...
			for (final RouteSpecificationSegment routeSpecificationSegment : pragmatachRoute.getRouteSpecification().getSegments()) {
				...
				if (null != staticPathPart) {
					if ((crackedURI[j].compareTo(staticPathPart) != 0)) {
						return false;
					}
				}
				j++;
	...
	private static boolean staticWildcardPartsMatch(PragmatachRoute pragmatachRoute, String[] crackedURI) throws PragmatachException {
		try {
			...
			for (int i = 0; i < (routeSpecificationSegments.size() - 1); i++) {
				...
				final String staticPathPart = routeSpecificationSegment.getPath();
				if (null != staticPathPart) {
					if ((crackedURI[i].compareTo(staticPathPart) != 0)) {
						return false;
					}
				}
			}
	...
	/**
	 * the method parameters. key is the id specified in the routespecification, value is the value
	 * passed in the uri
	 */
	...
	/**
	 * check that the number of segments matches and that each segment matches the regex, if there is
	 * one
	 */
			...
			if (!pragmatachRoute.isWildcardRoute()) {
				...
				if (null != crackedURI) {
					...
					if (crackedURI.length == pragmatachRoute.getSegmentCount()) {
						 ...
						 * check that the static path parts match
						 */
						if (!staticPartsMatch(pragmatachRoute, crackedURI)) {
							return false;
						}
						/*
						 * everything matches
				 ...
				 * check that the static path parts match
				 */
				if (!staticWildcardPartsMatch(pragmatachRoute, crackedURI)) {
					return false;
				}
				/*
				 * matches
