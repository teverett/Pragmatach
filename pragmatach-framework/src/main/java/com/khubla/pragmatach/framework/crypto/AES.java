...
public class AES {
   ...
   public static String decrypt(String encryptedPayload, String key) throws Exception {
      try {
         final SecretKeySpec secretKeySpec = new SecretKeySpec(makeAESKey(key), "AES");
         final Cipher cipher = Cipher.getInstance(KEYSPEC);
         cipher.init(Cipher.DECRYPT_MODE, secretKeySpec);
         final ByteArrayOutputStream baos = new ByteArrayOutputStream();
         final CipherInputStream CipherInputStream = new CipherInputStream(new ByteArrayInputStream(Base64.decodeBase64(encryptedPayload.getBytes())), cipher);
   ...
   public static String encrypt(String payload, String key) throws Exception {
      try {
         ...
         final CipherInputStream CipherInputStream = new CipherInputStream(new ByteArrayInputStream(payload.getBytes()), cipher);
         final ByteArrayOutputStream baos = new ByteArrayOutputStream();
         IOUtils.copy(CipherInputStream, baos);
   ...
   private static byte[] makeAESKey(String key) throws Exception {
      try {
         ...
         final byte[] rawkey = sha.digest(key.getBytes());
