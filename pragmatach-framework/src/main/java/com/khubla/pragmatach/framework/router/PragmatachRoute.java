...
import com.khubla.pragmatach.framework.url.*;
...
public class PragmatachRoute implements Comparable<PragmatachRoute> {
	...
	public static boolean scopes(String uri1, String uri2) {
		...
		if (uri1.compareTo("/*") == 0) {
			...
			if (isWildcardURI(uri1) && !isWildcardURI(uri2)) {
				/*
				 * uri1 is a wildcard and uri2 is specific.
				 */
				return true;
			} else if (isWildcardURI(uri2) && !isWildcardURI(uri1)) {
				/*
	...
	private void checkRouteSpecificationSanity() throws PragmatachException {
		try {
			...
			if (!isWildcardRoute()) {
				 ...
				 */
				if (((method.getParameterTypes().length != 0) || (0 != routeSpecification.getIds().size()))) {
					/*
					...
					/*
					 * check that the number of supplied variable bindings annotations matches the number
					 * of parameters
					 */
					...
					if ((null != routeParameters) && (routeParameters.size() > 0)) {
						for (final RouteParameter routeParameter : routeParameters) {
							...
							if (!routeSpecification.getIds().contains(boundName)) {
				...
				if (1 != method.getParameterTypes().length) {
					throw new PragmatachException(
							"Parameter number mismatch.  Method '" + method.getDeclaringClass().getName() + ":" + method.getName() + "' is bound to a wildcard route and must be a a single parameter");
				}
			}
	...
	/**
	 * get a class instance of the controller, and return a proxy that allows us to intercept method
	 * calls
	 */
