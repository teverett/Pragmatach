...
public class PragmatachServlet extends HttpServlet {
    ...
    */
	 ...
	 */
	private void addCustomResponseHeaders(Request request,
		 ...
		 * add
		 */
		httpServletResponse.addHeader(RENDERTIME_HEADER,
				Long.toString(rendertime));
		/*
		 * record it too
		 */
	...
	protected void doGet(HttpServletRequest httpServletRequest,
		...
		try {
			...
			final Request request = new Request(httpServletRequest,
					httpServletResponse, Route.HttpMethod.get,
					getServletConfig());
	...
	protected void doPost(HttpServletRequest httpServletRequest,
		...
		try {
			final Router requestRouter = new Router();
			final Request request = new Request(httpServletRequest,
					httpServletResponse, Route.HttpMethod.post,
					getServletConfig());
			final Response response = requestRouter.route(request);
			addCustomResponseHeaders(request, httpServletResponse);
			processResponse(response, httpServletResponse);
			...
			if (null != response) {
				...
				if (null != headers) {
					for (final Entry<String, String> entry : headers.entrySet()) {
						httpServletResponse.setHeader(entry.getKey(),
								entry.getValue());
					}
				}
