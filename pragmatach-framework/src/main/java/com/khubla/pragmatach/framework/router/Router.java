...
public class Router {
	 ...
	 */
	private static final LRUCache<String, RouteFinder> routeCache = new LRUCache<String, RouteFinder>(
			getRouteCacheSize());
	/**
	...
	private static int getRouteCacheSize() {
		try {
			...
			final String configuredSize = Application.getConfiguration()
					.getParameter("pragmatach.routecache.size");
	...
	/**
	 * get instance of Pragmatawch controller. Either a new request controller
	 * or an existing session controller
	 */
	private PragmatachController getPragmatachControllerInstance(
			PragmatachRoute pragmatachRoute, Request request)
			throws PragmatachException {
		try {
			final Class<?> clazz = pragmatachRoute.getMethod()
					.getDeclaringClass();
			...
			if (null != controller) {
				if (controller.scope() == Controller.Scope.request) {
					 ...
					 * session scope
					 */
					PragmatachController pragmatachController = SessionScopedControllers
							.getController(request.getSession(), clazz);
					if (null == pragmatachController) {
						pragmatachController = pragmatachRoute
								.getControllerClazzInstance(request);
						SessionScopedControllers.setController(
								request.getSession(), pragmatachController);
					}
					return pragmatachController;
				}
			} else {
				throw new PragmatachException("Class '" + clazz
						+ "' does not have an @Controller annotation");
			}
		} catch (final Exception e) {
			throw new PragmatachException(
					"Exception in getPragmatachControllerInstance", e);
		}
	}
	 ...
	 */
	private void informLifecycleListenersOfBeginRequest(
			PragmatachRoute pragmatachRoute, Request request) {
		for (final LifecycleListener lifecycleListener : Application
				.getLifecyclelisteners().getLifecycleListeners()) {
	 ...
	 */
	private void informLifecycleListenersOfEndRequest(
			PragmatachRoute pragmatachRoute, Request request) {
		for (final LifecycleListener lifecycleListener : Application
				.getLifecyclelisteners().getLifecycleListeners()) {
	 ...
	 */
	private Response invoke(PragmatachRoute pragmatachRoute, Request request,
			LinkedHashMap<String, String> parameterMap)
			throws PragmatachException {
		try {
			/*
			 * ret
			 ...
			 * get the controller
			 */
			final PragmatachController pragmatachController = getPragmatachControllerInstance(
					pragmatachRoute, request);
			/*
			 * set the route
			 */
			...
			final Class<?>[] methodParameterTypes = pragmatachRoute.getMethod()
					.getParameterTypes();
			/*
			 * wildcard?
			 */
			if (false == pragmatachRoute.isWildcardRoute()) {
				if ((null == methodParameterTypes)
					 ...
					 */
					int i = 0;
					for (final RouteParameter routeParameter : pragmatachRoute
						...
						if (parameterMap.containsKey(boundName)) {
							 ...
							 */
							final String parameterValue = parameterMap
									.get(boundName);
							params[i] = ConvertUtils.convert(parameterValue,
					 ...
					 */
					ret = (Response) method
							.invoke(pragmatachController, params);
				}
			} else {
				/*
				 * parameters to pass
	 ...
	 */
	private void processFormData(PragmatachController pragmatachController)
			throws PragmatachException {
		try {
			/*
			 * walk the fields
	 ...
	 */
	private void processParameterData(Request request,
		...
		try {
			 ...
			 * walk the fields
			 */
			final Map<String, String[]> parameterValues = request
					.getParameters();
			for (final Entry<String, String[]> entry : parameterValues
					.entrySet()) {
				/*
				 * set the fields
				 */
	...
	public Response route(Request request) throws PragmatachException {
		try {
			...
			if (null != routeFinder) {
				 ...
				 * note a message
				 */
				logger.debug(request.getMethod() + " request for: "
						+ request.getURI() + " routed to "
						+ routeFinder.getPragmatachRoute().getDescription());
				/*
				 * inform lifecycle listeners of start of request
				 */
				informLifecycleListenersOfBeginRequest(
						routeFinder.getPragmatachRoute(), request);
				/*
				 * invoke
				 */
				try {
					return invoke(routeFinder.getPragmatachRoute(), request,
							routeFinder.getParameterMap());
				} catch (final Exception e) {
					throw e;
				} finally {
					/*
					 ...
					 */
					informLifecycleListenersOfEndRequest(
							routeFinder.getPragmatachRoute(), request);
				}
			} else {
				/*
				 * log a message
				 */
				logger.info(request.getMethod() + " request for: "
						+ request.getURI() + " could not be routed");
				/*
				 * no match, return 404
				 */
